...
choice /c 12349 /n /m "Nhập từ bàn phím: "
...
set /p _vi=<_vi.json
jq -r ".[%_j%]|.ncg" oldData.json > _ncg.json
set /p _ncg=<_ncg.json
rem Xóa phần thập phân
set /a _ncg=%_ncg%
set /a _sumNCG+=%_ncg%
set _ncg=               %_ncg%
jq -r ".[%_j%]|.crystal" oldData.json > _crystal.json
...
set _temp1=  %_node%
echo.[*]Sử dụng node	:%_temp1:~-2%                                                     NCG             CRYSTAL
cd %_cd%\batch\avatarAddress
type _temp.json
echo.==========
set _sumNCG=               %_sumNCG%
set _sumCRYSTAL=               %_sumCRYSTAL%
echo [*]Tổng		:                                           %_sumNCG:~-15%	%_sumCRYSTAL:~-15%
echo.==========
set /a _j+=-1
echo.[1..%_j%] Tracked Avatar +
echo.[100] Gửi NCG/Crystal
echo.[200] Xem danh sách cụ thể
echo.[300] Mở nhanh tất cả ví đang có
echo.[400] Quay lại Menu
set /a _j=%_i%
echo.
...
if %_pick%==100 (set "_pick=" & start %_cd%\batch\SendCurrency.bat & goto :done)
